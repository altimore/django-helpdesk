...
from helpdesk.forms import (EditFollowUpForm, EditTicketForm, EmailIgnoreForm,
                            TicketCCEmailForm, TicketCCForm, TicketCCUserForm,
                            TicketDependencyForm, TicketForm, UserSettingsForm)
...
def _has_access_to_queue(user, queue):
    ...
    if (
        user.is_superuser
        or not helpdesk_settings.HELPDESK_ENABLE_PER_QUEUE_STAFF_PERMISSION
    ):
        return True
    else:
...
def dashboard(request):
    ...
    unassigned_tickets = (
        Ticket.objects.select_related("queue")
        .filter(assigned_to__isnull=True, queue__in=user_queues)
    ...
    if email_current_user:
        all_tickets_reported_by_current_user = (
            Ticket.objects.select_related("queue")
            .filter(submitter_email=email_current_user)
            .order_by("status")
        )
...
def subscribe_staff_member_to_ticket(ticket, user):
    ...
    ticketcc = TicketCC(ticket=ticket, user=user, can_view=True, can_update=True)
    ticketcc.save()
...
def update_ticket(request, ticket_id, public=False):
    ...
    if title and title != ticket.title:
        c = TicketChange(
            followup=f, field=_("Title"), old_value=ticket.title, new_value=title
        )
        c.save()
        ticket.title = title
    ...
    if due_date != ticket.due_date:
        c = TicketChange(
            followup=f, field=_("Due on"), old_value=ticket.due_date, new_value=due_date
        )
        c.save()
    ...
    if (
        ...
        if (
            ...
            or (
                reassigned
                and ticket.assigned_to.usersettings_helpdesk.settings.get(
...
def ticket_list(request):
            ...
            if six.PY3:
                if DJANGO_VERSION[0] > 1:
                    ...
                    query_params = json.loads(
                        b64decode(str(saved_query.query).lstrip("b\\'")).decode()
                    )
                else:
                    query_params = json.loads(
                        b64decode(str(saved_query.query)).decode()
                    )
            else:
    ...
    search_message = ""
    if "query" in context and settings.DATABASES["default"]["ENGINE"].endswith(
        "sqlite"
    ):
        search_message = _(
    ...
    return render(
        ...
        dict(
            context,
            tickets=ticket_qs,
            default_tickets_per_page=request.user.usersettings_helpdesk.settings.get(
                "tickets_per_page"
            )
            or 25,
...
def create_ticket(request):
    ...
    if request.method == "POST":
        ...
        initial_data = {}
        if (
            request.user.usersettings_helpdesk.settings.get(
...
def hold_ticket(request, ticket_id, unhold=False):
    ...
    f = FollowUp(
        ticket=ticket, user=request.user, title=title, date=timezone.now(), public=True
    )
    f.save()
...
def run_report(request, report):
    ...
    report_queryset = (
        Ticket.objects.all()
        .select_related()
        .filter(queue__in=_get_user_queues(request.user))
    )
...
def ticket_cc_add(request, ticket_id):
    ...
    return render(
        ...
        {"ticket": ticket, "form_email": form_email, "form_user": form_user},
    )
...
def ticket_dependency_add(request, ticket_id):
    ...
    return render(
        request, "helpdesk/ticket_dependency_add.html", {"ticket": ticket, "form": form}
    )
...
def calc_basic_ticket_stats(Tickets):
    ...
    ots.append(
        ["Tickets < 30 days", N_ota_le_30, "success", sort_string(date_30_str, "")]
    )
    ots.append(
        [
            "Tickets 30 - 60 days",
