...
urlpatterns = [
    ...
    url(r"^tickets/(?P<ticket_id>[0-9]+)/edit/$", staff.edit_ticket, name="edit"),
    url(r"^tickets/(?P<ticket_id>[0-9]+)/update/$", staff.update_ticket, name="update"),
    url(r"^tickets/(?P<ticket_id>[0-9]+)/delete/$", staff.delete_ticket, name="delete"),
    url(r"^tickets/(?P<ticket_id>[0-9]+)/hold/$", staff.hold_ticket, name="hold"),
    url(r"^tickets/(?P<ticket_id>[0-9]+)/unhold/$", staff.unhold_ticket, name="unhold"),
    url(r"^tickets/(?P<ticket_id>[0-9]+)/cc/$", staff.ticket_cc, name="ticket_cc"),
    url(
    ...
    url(r"^raw/(?P<type>\w+)/$", staff.raw_details, name="raw"),
    url(r"^rss/$", staff.rss_list, name="rss_index"),
    url(r"^reports/$", staff.report_index, name="report_index"),
    url(r"^reports/(?P<report>\w+)/$", staff.run_report, name="run_report"),
    url(r"^save_query/$", staff.save_query, name="savequery"),
    url(
        r"^delete_query/(?P<id>[0-9]+)/$", staff.delete_saved_query, name="delete_query"
    ),
    url(r"^settings/$", staff.EditUserSettingsView.as_view(), name="user_settings"),
...
urlpatterns += [
    url(r"^$", protect_view(public.Homepage.as_view()), name="home"),
    url(r"^tickets/submit/$", public.create_ticket, name="submit"),
    url(
        r"^tickets/submit_iframe/$",
        public.CreateTicketIframeView.as_view(),
        name="submit_iframe",
    ),
    url(
        r"^tickets/success_iframe/$",  # Ticket was submitted successfully
        public.SuccessIframeView.as_view(),
        name="success_iframe",
    ),
    url(r"^view/$", public.view_ticket, name="public_view"),
    url(r"^change_language/$", public.change_language, name="public_change_language"),
]
...
if helpdesk_settings.HELPDESK_KB_ENABLED:
    urlpatterns += [
        url(r"^kb/$", kb.index, name="kb_index"),
        url(r"^kb/(?P<slug>[A-Za-z0-9_-]+)/$", kb.category, name="kb_category"),
        url(r"^kb/(?P<item>[0-9]+)/vote/$", kb.vote, name="kb_vote"),
        url(
